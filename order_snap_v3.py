...
def write_snapshot(snap, offset, last):
	...
	if (DEBUG>0):  print ('Task: {0:0d} - ordered snapshots written to file, redshift = {4:0.2f}, offset = {1:0d}, last = {2:0d}, time = {3:%H:%M:%S}'.format(rank, offset, last, datetime.datetime.now(),head.redshift))
...
#input file 
snapshot_fname = '/cosma6/data/dp004/dc-smit4/Daemmerung/Planck2013-Npart_2048_Box_3000-Fiducial/run1/snapdir_{0:03d}/Planck2013-L3000-N2048-Fiducial_{0:03d}'.format(start_snap)	
...
# block determines which COSMA node (0 or 1) contains current MPI task
if (rank<28):
	mb = 0
	block = 0
else:
...
# determine first and last sub-snapshot file to be read by this task
chunk = size//2  # this is number of cores participating in ths file read operation
brank = rank - mb
batch = filenum//chunk #number of sub-snapshot files in each batch
rem = filenum%chunk
start_file = brank * batch
if (brank == chunk-1):
	end_file = start_file + batch + rem
else:
	end_file = start_file + batch 
...
for snap in range(start_snap, end_snap):
	...
	# read each sub-snapshot file in this snapshot	
	for i in range(start_file,end_file):  		
		if (DEBUG>1) & ((i==start_file) | (i==end_file-1)):  print ('Task: {0:d}, Sub-snapshot {1:d}, time = {2:%H:%M:%S}'.format(rank, i, datetime.datetime.now()))
		read_subsnapshot(i, block)

	# wait for all tasks to complete
	comm.Barrier()
	if (DEBUG>0 and rank==0):  print ('Completed processing snapshot {0:d},  redshift = {1:0.2f}, time =  {2:%H:%M:%S}'.format(snap, head.redshift, datetime.datetime.now()))

	# write ordered snapshots to file
	write_snapshot(snap, offset, last)
		
	# wait for all tasks to complete before contimuing to next snapshot
	comm.Barrier()
	if (DEBUG>0 and rank==0):  print ('Completed writing snapshot {0:d}, time = {1:%H:%M:%S}'.format(snap, datetime.datetime.now()))
	
if (DEBUG>0 and rank==0):  print ('Completed writing all snapshots, time = {0:%H:%M:%S}'.format(datetime.datetime.now()))